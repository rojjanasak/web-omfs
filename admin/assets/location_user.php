   ...
   $data = $_GET['data'];
    $val = $_GET['val'];
          ...
          if ($data=='prov_name') { 
              ...
              $result=pg_query("select pv_tn,pv_code from tambon_list GROUP BY pv_tn,pv_code order by pv_tn asc");
              ...
              $result=pg_query("SELECT ap_tn,ap_code FROM tambon_list WHERE pv_tn= '$val' GROUP BY ap_tn,ap_code ");
              while($row = pg_fetch_array($result)){
              ...
              $result=pg_query("SELECT tb_tn,tb_code FROM tambon_list WHERE ap_tn = '$val' GROUP BY tb_tn,tb_code");
              while($row = pg_fetch_array($result)){
